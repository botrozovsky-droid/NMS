...
class DatabaseAdapter {
  ...
  queryNodes(filters = {}) {
    ...
    if (filters.is_manual !== undefined) {
      query += ' AND is_manual = ?';
      params.push(filters.is_manual ? 1 : 0);
    }
  ...
  getNodeEdges(nodeId) {
    ...
    return rows.map(row => this._deserializeEdge(row));
  }
  ...
  _serializeNode(node) {
    return [
      ...
      node.confidence || 0.7,
      node.activations || 0,
      node.lastActivation || node.last_activation || null,
      node.created,
      node.extractionType || node.extraction_type || null,
      node.rationale || null,
      node.canonicalForm || node.canonical_form || null,
      JSON.stringify(node.aliases || []),
      JSON.stringify(node.flags || []),
      (node.is_manual !== undefined ? node.is_manual : node.isManual) ? 1 : 0,
  ...
  _deserializeNode(row) {
    return {
      ...
      is_manual: row.is_manual === 1,  // Include both for compatibility
  ...
  _serializeEdge(edge) {
    return [
      ...
      edge.weight || 0.0,
      edge.created,
      edge.lastUpdated || edge.last_updated || null,
      edge.rationale || null,
